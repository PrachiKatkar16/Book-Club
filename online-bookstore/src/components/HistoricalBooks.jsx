// src/components/HistoricalBooks.jsx
...
const HistoricalBooks = ({ setCategory }) => {
...
`;
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      setLoading(true);
      try {
        const response = await axios.get('https://www.googleapis.com/books/v1/volumes?q=subject:history');
  ...
  return (
    <div>
      ...
      <Box p="4" border={'1px solid'} width={'1000px'} position='relative' left={'-40%'}>
        <Grid templateColumns={{ base: 'repeat(1, 1fr)', md: 'repeat(2, 1fr)', lg: 'repeat(3, 1fr)' }} gap={'10px'}>
          {books.map((book) => (
            <RouterLink to={`/book/${book.id}`} key={book.id}>
              <Box borderWidth="1px" borderRadius="lg" overflow="hidden" p="4" textAlign={'center'}>
                {book.volumeInfo.imageLinks?.thumbnail && (
                  ...
                  <Image src={book.volumeInfo.imageLinks.thumbnail} alt={book.volumeInfo.title} mb="4" align={'center'}/>
                  </Box>
                )}
